...
module.exports = class extends Generator {
  constructor(args, options) {
    ...
    this.option('theme-name', { type: String, desc: 'The theme machine name' });

    // Allow the user to pass in a component name.
    this.option('name', { type: String, desc: 'The component name' });

    // Allow the user to opt in to a JS behavior file.
    this.option('include-js', { type: Boolean, desc: 'Include a JS file' });
  }
  ...
  // theme name and Component name. Without those we can't create a basic component.
  prompting() {

    // If we DO have both the theme name _and_ component name passed as arguments we can skip all the prompts.
    ...
    try {
      ...
      // If it does, read it and use the name as our default theme machine name.
    ...
    // If it doesn't, let the user know we can't continue. We need to run from the theme root.
    catch (err) {
      assert.fail(
        `
  ...
  writing() {
    ...
    // Build out the library structure so we can append it to the libraries.yml file.
    ...
    // Add a blank line so the file is nicely formatted and the appended data doesn't run into the current data within the file.
    fs.appendFileSync(
    ...
    // Write each file the component needs, adding the component name where needed.
    this.fs.copyTpl(
